...
let europeCredit;
let europeInsurance;
let europePrivateEquity;
let europeSpecialSituation;
let europeTechOpp;
...
export function setupEnvironment() {
  beforeEach("Setup Test Environment", function () {
    ...
    cy.fixture('stagingAuthentication.json').then((credentials) => {
      // Use the credentials dynamically to construct the URL
      const { username, password } = credentials;
      cy.visit(`https://${username}:${password}@bccorp-staging.lousch.agency`);
      cy.wait(5000)
    })
    ...
    cy.fixture("urlTestData.json").then((data) => {
      urls = data;
    });

    /*************************** */
    cy.fixture("noLocationFoundTestData.json").then((data) => {
      nolocation = data;
    });
    cy.fixture("locations/europeLocation/europeAllTestData.json").then(
      (data) => {
        europeAll = data;
      }
    );
    cy.fixture("locations/europeLocation/europeCreditTestData.json").then(
      (data) => {
        europeCredit = data;
      }
    );
    cy.fixture("locations/europeLocation/europeInsuranceTestData.json").then(
      (data) => {
        europeInsurance = data;
      }
    );
    cy.fixture("locations/europeLocation/europePrivateEquityTestData.json").then(
      (data) => {
        europePrivateEquity = data;
      }
    );
    cy.fixture("locations/europeLocation/europeSpecialSituationTestData.json").then(
      (data) => {
        europeSpecialSituation = data;
      }
    );
    cy.fixture("locations/europeLocation/europeTechOppTestData.json").then(
      (data) => {
        europeTechOpp = data;
      }
    );
